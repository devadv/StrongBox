...
public class Controller {
	...
	public Controller(Model model) {
		...
		((AbstractDocument) view.getFields().get(3).getDocument())
				.setDocumentFilter(new PasswordFilter(view.getFields().get(3)));
	...
	public void addRecordListener() {
		...
		view.getRecordView().getSelectionModel()
				.addListSelectionListener(new ListSelectionListener() {
					public void valueChanged(ListSelectionEvent e) {
						if (!e.getValueIsAdjusting()) {
							try {
								record = view.getRecordView()
										.getSelectedValue();
								String[] fields = model.getRecordFields(record);
								for (int i = 0; i < 6; i++) {
									view.setDarkGrayColor(view.getFields().get(
											i));
									view.getFields().get(i).setText(fields[i]);
								}
								view.getStrengthScoreLabel().setText(
										""
												+ PasswordSafe.getScore(record
														.getPassword()));
							} catch (NullPointerException exc) {
								for (int i = 0; i < 6; i++) {
									view.setPlainGrayColor(view.getFields()
											.get(i));
									view.getFields().get(i)
											.setText("No record selected");
								}
	...
	/**
	 * Enable or disable the scrollpanes with the JLists, and the 5 buttons plus
	 * searchbox at the top. Toggle icon- and color-state as well.
	 * 
	 * @param b
	 *            If true these components are enabled, if false they are
	 *            disabled. Also toggle icons and colors accordingly.
	 */
	public void setEnableNormalMode(boolean b) {
		...
		if (b) {
			...
			for (int i = 0; i < 5; i++) {
				view.getIconButton(i).removeMouseListener(
						view.getIconButton(i).getMouseListeners()[0]);
	...
	/**
	 * Enable or disable the "dice" button, the save and cancel buttons, the
	 * textfields and toggle the text color under the buttons.
	 * 
	 * @param b
	 *            If true these components are enabled, if false they are
	 *            disabled. Also toggle icons and colors accordingly.
	...
	public void setEnableEditMode(boolean b) {
		...
		if (b) {
			...
			view.getIconButton(6).removeMouseListener(
					view.getIconButton(6).getMouseListeners()[0]);
	 ...
	 *             if user typed comma's into one of the fields.
	 */
		...
		view.getButton(0).addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					view.showMessageDialog(messages.getDialog(0),
							"Illegal Arguments", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	}
			...
			public void actionPerformed(ActionEvent e) {
				if (view.showConfirmDialog("Discard changes?")) {
					...
					try {
						...
						if (edit) {
							view.getStatusLabel()
									.setText(messages.getStatus(9));
							anim.slowFade();
						} else {
							view.getStatusLabel().setText(
									messages.getStatus(10));
							anim.slowFade();
						}
						initSearchBox();
						view.getRecordView().grabFocus();
	...
	/**
	 * Add an ActionListener to the 'delete' button so selected records can be
	 * deleted (ask user for confirmation) from the records-list.
	 */
	public void addDeleteListener() {
		view.getIconButton(2).addMouseListener(new MouseListener() {
				...
				if (view.getRecordView().getSelectedValue() != null) {
					...
					if (view.showConfirmDialog("Are you sure you want to"
						...
						if (folderData.indexOf(folder) != -1) {
							view.getFolderView().setSelectedIndex(
									folderData.indexOf(folder));
						}
					}
				} else {
					view.showMessageDialog("No record selected");
			...
			public void mouseExited(MouseEvent e) {
				...
				if (!view.getStatusLabel().getText()
						.equals(messages.getStatus(5))) {
					anim.fastFade();
				}
			}
	...
	/**
	 * Add a MouseListener to the 'Eye' button so the user can show or hide the
	 * password. Bullet symbols are used to mask the password.
	 */
	public void addEyeButtonListener() {
		view.getIconButton(5).addMouseListener(new MouseListener() {
			public void mouseClicked(MouseEvent e) {
				...
				JPasswordField pwField = (JPasswordField) view.getFields().get(
						3);
				if (!showPassword) {
	...
	public void addDiceButtonListener() {
		view.getIconButton(6).addMouseListener(new MouseListener() {
			public void mouseClicked(MouseEvent e) {
				view.getIconButton(6).setIcon(view.getIcon(7));
				view.getFields().get(3)
						.setText(PasswordSafe.generatePassphrase(12));
				view.getStrengthScoreLabel().setText(
						""
								+ PasswordSafe.getScore(view.getFields().get(3)
										.getText()));
			}
	...
	/**
	 * Copy the login name to the system clipboard when the user right-clicks on
	...
	/**
	 * Copy the password to the system clipboard when the user right-clicks on
	 * the password-field.
	 */
	 ...
	 * 
	 * @param doc
	 *            The document of which it's content will be used to search the
	 *            list with records.
	 */
	 ...
	 *             if a portion of the given range was not a valid part of the
	 *             document.
	 */
	 ...
	 * the password safety score. Keep the text from the pwStrength Label
	 * up-to-date with this score and also assign an appropriate description and
	 * color to the label based on the score.
	 */
	public class PasswordFilter extends DocumentFilter {
		...
		public PasswordFilter(JTextField field) {
			...
			this.pwScore = PasswordSafe.getScore(Controller
					.getDocumentText(doc));
		}

		public void setStrengthLabel() {
			view.getStrengthScoreLabel().setForeground(
					PasswordSafe.getScoreColor(pwScore));
